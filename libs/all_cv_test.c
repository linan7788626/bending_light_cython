...
void xy_rotate(double *x1_in,double *x2_in,int nx1,int nx2,double xc1,double xc2,double pha,double *x1_out,double *x2_out) {
	...
	int i,j,index;
#pragma omp parallel num_threads(4)	\
	shared(x1_in,x2_in,nx1,nx2,xc1,xc2,phirad,x1_out,x2_out) \
	private(i,j,index)
	{
	#pragma omp for schedule(dynamic,16)
...
void gauss_2d(double *x1,double *x2,int nx1,int nx2,double *par,double *res) {
	...
	int i,j,index;
#pragma omp parallel num_threads(4)	\
	shared(x1new,x2new,nx1,nx2,par,res) \
	private(i,j,index)
	{
	#pragma omp for schedule(dynamic,16)
		for (i=0;i<nx1;++i) for (j=0;j<nx2;++j) {
...
void lq_nie(double *x1,double *x2,int nx1,int nx2,double *lpar,double *alpha1,double *alpha2) {
	...
	double xt1,xt2,phi,a1,a2;

	int i,j,index;
#pragma omp parallel num_threads(16)	\
	shared(x1,x2,nx1,nx2,cosa,sina,q,rc,re) \
	private(i,j,index,xt1,xt2,phi,a1,a2)
	{
	#pragma omp for schedule(dynamic,64)
		for (i = 0;i<nx1;++i) for (j = 0;j<nx2;++j) {
			index = i*nx2+j;
			xt1 = (x1[index]-xc1)*cosa+(x2[index]-xc2)*sina;
    		xt2 = (x2[index]-xc2)*cosa-(x1[index]-xc1)*sina;
